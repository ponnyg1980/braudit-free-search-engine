...
def _fuzzy_points(mark_u: str, phrase: str) -> int:
    ...
    for r in ratios:
        if r >= FUZZY_STRONG_RATIO:
            best = max(best, SIM_FUZZY_STRONG)
        elif r >= FUZZY_WEAK_RATIO:
            best = max(best, SIM_FUZZY_WEAK)
    return best


def _phonetic_points(mark_u: str, phrase: str) -> int:
    ...
    for cand in candidates:
        ...
        if cand_norm == target_norm:
            return SIM_FUZZY_STRONG
        if target_sdx and _soundex(cand) == target_sdx:
            return SIM_FUZZY_STRONG
        if _fuzzy_ratio(cand_norm, target_norm) >= FUZZY_STRONG_RATIO:
            return SIM_FUZZY_STRONG

    # Nothing strong; allow a weak award for a near-miss on the whole string.
    if _fuzzy_ratio(_phonetic_normalise(mark_u), target_norm) >= FUZZY_WEAK_RATIO:
        return SIM_FUZZY_WEAK
    return 0
...
def mark_similarity_components(mark_u: str, word_searches) -> list[dict]:
    ...
    for ws in word_searches or []:
        ...
        if stype == "exact match":
            hit = mark_u == phrase
            emit("exact", SIM_EXACT if hit else 0,
...
def _mark_similarity_points(mark_u: str, word_searches) -> tuple[int, str]:
    ...
    for c in mark_similarity_components(mark_u, word_searches):
        if c["points"] > best:
...
def risk_from_score(score: int, status: str) -> str:
    ...
    if _clean(status).lower() in DEAD_STATUSES:
        return RESULT_ONLY
    if score >= WORD_HIGH_MIN:
        return HIGH
    if score >= WORD_MEDIUM_MIN:
        return MEDIUM
    if score >= 6:
        return LOW_MEDIUM
    return LOW
...
def score_word_result(result: dict, criteria: dict) -> dict:
    ...
    # --- mark similarity ---
    sim_pts, sim_why = _mark_similarity_points(mark_u, criteria.get("word_searches"))
    ...
    for ws in (criteria.get("word_searches") or []):
        ...
        if not phrase:
            continue
        pts, _ = _mark_similarity_points(mark_u, [ws])
        if pts > best_pts:
