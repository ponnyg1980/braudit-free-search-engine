...
function render(){const run=D.run,req=run.request||{},rp=D.requested;
 ...
 renderFindings();renderExcluded();renderForensicLog();renderS4();renderSettings();
...
function renderChannel(c){const rows=D.results.filter(x=>x.channel===c);const st=state[c]||(state[c]={status:'new',groups:new Set(['high','medium','low','cleared']),q:'',wide:false});
 ...
 if(st.wide){const cols=[];for(const x of view)for(const k of Object.keys(flat(x)))if(!cols.includes(k))cols.push(k);
  box.className='wide';box.innerHTML='<table id="t"><tr><th></th><th>Assessment</th><th>Review</th><th>Action</th>'+cols.map(k=>`<th>${esc(k)}</th>`).join('')+'</tr>'+view.map(x=>{const f=flat(x);return `<tr class="${x.review_status==='excluded'?'row-ex':''}"><td><input type="checkbox" data-id="${x.id}" ${(sel[c]||new Set()).has(x.id)?'checked':''}></td><td><span class="band ${bc(x.band)}">${esc(x.band||'—')}</span></td><td>${revHtml(x)}</td><td style="min-width:110px">${actHtml(x)}</td>`+cols.map(k=>`<td>${/^https?:\/\//i.test(String(f[k]||''))?(k==='image_url'?`<a target="_blank" href="${esc(f[k])}"><img class="thumb" src="${esc(f[k])}"></a>`:link(f[k])):clamp(String(f[k]??''))}</td>`).join('')+'</tr>'}).join('')+'</table>';
...
function rowHtml(c,x){const d=x.detail||{},dt=x.dates||{};let a,b2,c3;
 ...
 const tail=`<td><span class="band ${bc(x.band)}">${esc(x.band||'—')}</span><span class="meta">score ${x.score??''}</span></td><td class="why">${clamp(x.explanation||'')}</td><td class="why" style="min-width:160px;font-size:11.5px">${(x.found_by||[]).map(esc).join('<br>')}</td><td>${revHtml(x)}</td><td style="min-width:110px">${actHtml(x)}</td></tr>`;
