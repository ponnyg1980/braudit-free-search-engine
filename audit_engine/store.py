...
class Store:
    ...
    # Decision H. An append-only event log: applying, amending and clearing all
    # INSERT, and the row in force is the one with superseded_at null. Nothing
    # here writes a global default, and nothing here touches audit.results —
    # the run's own scoring is a historical fact and the sensitivity is a lens
    # over it. Exclusions and review verdicts are untouched in every path,
    # deliberately: a client's judgement must never be collateral damage from
    # a scoring decision.

    def sensitivity(self, run_id: str) -> dict | None:
        """What this run is being read at right now. None means standard."""
        with self._conn() as c:
            row = c.execute(
                """select * from audit.run_sensitivity
                    where run_id = %s and superseded_at is null""",
                (run_id,)).fetchone()
        if not row or not (row.get("steps") or {}):
            # A cleared run has a live row with empty steps. It is history, not
            # a setting, so the caller is told "standard" and reads the history
            # if it wants to know that it was once otherwise.
            return None
        return row

    def sensitivity_history(self, run_id: str) -> list[dict]:
        """Every event, newest first. This is the thing worth keeping.

        "fuzzy +1, then +2 by Sarah at 14:20, cleared by Tom at 16:05 because
        the client disputed the list" is what R&D rules a global change on, and
        it only exists because nothing here updates in place.
        """
        with self._conn() as c:
            return c.execute(
                """select id, version, action, steps, prior_steps, deltas,
                          rows_moved, moved_up, moved_down, band_before, band_after,
                          reason, created_by, scoring_version, created_at, superseded_at
                     from audit.run_sensitivity
                    where run_id = %s order by version desc""",
                (run_id,)).fetchall()

    def save_sensitivity(self, run_id: str, steps: dict, by: str,
                         comparison: dict | None = None,
                         reason: str | None = None,
                         action: str | None = None) -> dict:
        """Append one event and retire the previous one, in a single transaction.

        `action` is inferred when not given: the first event is 'applied', a
        later one with steps is 'amended', and one with no steps is 'cleared'.

        The measured consequence travels with the event rather than being
        recomputed later, because the numbers the person was SHOWN when they
        decided are the numbers the log should carry. Re-deriving them from a
        newer package would quietly rewrite what they agreed to.
        """
        steps = dict(steps or {})
        cmp_ = dict(comparison or {})
        with self._conn() as c:
            live = c.execute(
                """select version, steps from audit.run_sensitivity
                    where run_id = %s and superseded_at is null for update""",
                (run_id,)).fetchone()
            ...
            if live:
                c.execute(
                    """update audit.run_sensitivity set superseded_at = now()
                        where run_id = %s and superseded_at is null""", (run_id,))
            row = c.execute(
                """insert into audit.run_sensitivity
                       (run_id, version, action, steps, prior_steps, deltas,
                        rows_moved, moved_up, moved_down, band_before, band_after,
                        reason, created_by, scoring_version)
                   values (%s, %s, %s, %s::jsonb, %s::jsonb, %s::jsonb,
                           %s, %s, %s, %s::jsonb, %s::jsonb, %s, %s, %s)
                returning id, version, action, created_at""",
                (run_id, version, action,
                 Json(steps), Json(prior), Json(cmp_.get("deltas") or {}),
                 cmp_.get("rows_moved"), cmp_.get("moved_up"), cmp_.get("moved_down"),
                 Json(cmp_.get("band_before") or {}), Json(cmp_.get("band_after") or {}),
                 (reason or None), by, cmp_.get("scoring_version") or _scoring_version()),
            ).fetchone()
        return {"id": str(row["id"]), "version": row["version"],
                "action": row["action"], "steps": steps, "prior_steps": prior}
