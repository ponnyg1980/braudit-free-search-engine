...
def _search_score_settings(st, run: dict, run_id: str) -> dict:
    ...
    try:
        ...
        package = None
    # Sensitivity (21 Sep 2026). The CONTROLS are described by the package,
    # not by this file: compartments, the positions each one offers and what
    # each position means all come from tmh_scoring.sensitivity, so a release
    # that adds a compartment or retires a step cannot leave the screen
    # disagreeing with the scorer.
    controls, live, history = [], None, []
    try:
        from tmh_scoring.sensitivity import (AVAILABLE_STEPS, PRESETS,
                                             STEP_LABELS)
        from tmh_scoring.settings import COMPARTMENT_LABELS
        live = st.sensitivity(run_id)
        history = st.sensitivity_history(run_id)
        current = (live or {}).get("steps") or {}
        for comp, steps in AVAILABLE_STEPS.items():
            controls.append({
                "compartment": comp,
                "label": COMPARTMENT_LABELS.get(comp, comp),
                "steps": [{"step": s, "label": STEP_LABELS[s],
                           "changes": PRESETS[comp][s]} for s in steps],
                "current": int(current.get(comp, 0)),
                # The image scorer is built but wired to no run yet, so its
                # steps would move nothing. Shown and disabled rather than
                # hidden, so the compartment set on screen is the real one.
                "enabled": comp != "visual",
            })
    except Exception:
        pass
...
def _apply_lens(st, run_id: str, results: list) -> dict | None:
    """Show this run's bands at the sensitivity saved on it.

    The stored rows are not touched — the run's own scoring is a historical
    fact, and the sensitivity is a lens over it. What changes is the band the
    SCREEN reads, and it changes in `band` rather than in a new field on
    purpose: every filter, sort, count and table on the triage page already
    keys off `band`, so a second field would mean the summary bar, the band
    filter and the findings list each having to remember which one to use, and
    one of them eventually forgetting. What was stored travels as
    `band_standard`, so nothing is hidden and the screen marks every row that
    moved.

    Returns None when the run is at the standard calibration, a dict
    describing the lens when it is not, and {"error": ...} if it could not be
    applied — which is reported on the page rather than swallowed, because a
    screen quietly showing unadjusted bands to someone who set a sensitivity
    is worse than a visible failure.
    """
    try:
        live = st.sensitivity(run_id)
        if not live or not (live.get("steps") or {}):
            return None
        from . import rescore
        from tmh_scoring.sensitivity import settings_for
        adjusted = rescore.rescore_run(st, run_id, settings_for(live["steps"]))
        moved = 0
        for r in results:
            new = adjusted.get(str(r["id"]), r.get("band"))
            r["band_standard"] = r.get("band")
            if new != r.get("band"):
                moved += 1
            # A row that falls to "no meaningful resemblance" at this
            # sensitivity is no longer a finding. It stays on the screen,
            # labelled, rather than vanishing: a result that disappeared
            # without explanation is how staff stop trusting the tool.
            r["band"] = new if new is not None else "Not a finding at this sensitivity"
        return {"steps": live["steps"], "version": live.get("version"),
                "created_by": live.get("created_by"), "reason": live.get("reason"),
                "rows_moved": moved}
...
def api_run(run_id: str, who: dict = Depends(staff)):
    ...
    payload_lens = _apply_lens(st, run_id, results)
    payload = {
        ...
        # Present and non-null means every band on this screen has been read
        # through a sensitivity. The banner is not optional.
        "lens": payload_lens,
    }
    return Response(_dumps(payload), media_type="application/json")


# --- sensitivity, at Triage/Client Review level (21 Sep 2026) --------------
# Decision H. Nothing below writes a global default: the only thing these
# endpoints can produce is a per-run delta from the shipped calibration, and
# moving the calibration is a tmh-scoring release. Nothing below writes to
# audit.results either — the run's own scoring is a historical fact and the
# sensitivity is a lens over it — and nothing touches exclusions or review
# verdicts, because a client's judgement must never be collateral damage from
# a scoring decision.


def _steps_from(body: dict) -> dict:
    """Validate the posted steps through the package, not by hand.

    `normalise_steps` raises on an unknown compartment, an out-of-range step
    or a position that compartment does not offer. Letting a typo through to
    score at standard while logging a change is the one failure this whole
    design exists to prevent, so the error is a 400 rather than a shrug.
    """
    from tmh_scoring.sensitivity import SensitivityError, normalise_steps
    raw = body.get("steps")
    if not isinstance(raw, dict):
        raise HTTPException(400, "steps must be an object, e.g. {\"phonetic\": 1}")
    try:
        return normalise_steps(raw)
    except SensitivityError as exc:
        raise HTTPException(400, str(exc))


@app.post("/api/runs/{run_id}/sensitivity/preview")
async def api_sensitivity_preview(run_id: str, request: Request, who: dict = Depends(staff)):
    """What these steps WOULD do. Changes nothing, stores nothing.

    The preview exists because "sensitivity applied" is not a decision and
    "this moves 43 rows up, 11 down and takes 12 off the report entirely" is.
    """
    from . import rescore
    body = await request.json()
    st = _store()
    ...
    return Response(_dumps(rescore.compare(st, run_id, _steps_from(body))),
                    media_type="application/json")


@app.post("/api/runs/{run_id}/sensitivity")
async def api_sensitivity_apply(run_id: str, request: Request, who: dict = Depends(staff)):
    """Apply or amend this run's sensitivity.

    The consequence is measured HERE rather than trusted from the preview the
    browser saw: a stale tab, or a second person applying something else in
    between, would otherwise log numbers that never happened.
    """
    from . import rescore
    body = await request.json()
    steps = _steps_from(body)
    if not steps:
        raise HTTPException(400, "no steps given — to return this run to the "
                                 "standard calibration use DELETE, which "
                                 "records why")
    st = _store()
    if not st.run(run_id):
        raise HTTPException(404, "no such run")
    cmp_ = rescore.compare(st, run_id, steps)
    saved = st.save_sensitivity(run_id, steps, who["name"], comparison=cmp_,
                                reason=(body.get("reason") or "").strip() or None)
    return Response(_dumps({**saved, "comparison": cmp_}),
                    media_type="application/json")


@app.delete("/api/runs/{run_id}/sensitivity")
async def api_sensitivity_clear(run_id: str, request: Request, who: dict = Depends(staff)):
    """Return this run to the standard calibration.

    The dangerous one, and treated as such. A reason is REQUIRED — the check
    is also a constraint on the table, so it cannot be bypassed by calling
    this differently. Clearing appends rather than deletes: the event carries
    the steps that were in force, so the history can still be read and the
    previous state re-applied from what it says.

    Exclusions are untouched. Said here as well as in the UI because it is the
    question staff will ask before they press it.
    """
    from . import rescore
    body = await request.json()
    reason = (body.get("reason") or "").strip()
    if len(reason) < 10:
        raise HTTPException(400, "clearing sensitivity needs a reason (at least "
                                 "10 characters) — it is recorded and it is "
                                 "what makes the change explainable later")
    st = _store()
    if not st.run(run_id):
        raise HTTPException(404, "no such run")
    live = st.sensitivity(run_id)
    if not live:
        raise HTTPException(409, "this run is already at the standard calibration")
    # Measured from what is in force back to standard, so the log says what
    # the clear COST rather than only that it happened.
    cmp_ = rescore.compare(st, run_id, live.get("steps") or {})
    cost = {**cmp_,
            "rows_moved": cmp_["rows_moved"],
            "moved_up": cmp_["moved_down"], "moved_down": cmp_["moved_up"],
            "dropped": cmp_["added"], "added": cmp_["dropped"],
            "band_before": cmp_["band_after"], "band_after": cmp_["band_before"]}
    saved = st.save_sensitivity(run_id, {}, who["name"], comparison=cost,
                                reason=reason, action="cleared")
    return Response(_dumps({**saved, "comparison": cost}),
                    media_type="application/json")


@app.get("/api/runs/{run_id}/sensitivity/history")
def api_sensitivity_history(run_id: str, who: dict = Depends(staff)):
    """Every event on this run, newest first."""
    return Response(_dumps(_store().sensitivity_history(run_id)),
                    media_type="application/json")
