...
def ignored_in(text: str, *, where: str) -> list[dict]:
    ...
    for tok in _tokens(text or ""):
        ...
        if low in seen:
            continue
        # 2.3.0: was LEGAL_FORMS_AUDIT, one of the two flat patterns that
        # caused finding G. is_structural() answers the same labelling
        # question from the per-jurisdiction lists, folded across every
        # jurisdiction — see the note on _all_structural().
        structural = _is_structural(tok)
