...
# ---------------------------------------------------------------------------
# Classification — a different question from stripping
# ---------------------------------------------------------------------------
# LEGAL_FORMS_AUDIT and LEGAL_FORMS_WATCH, the two flat patterns that had
# drifted apart and caused finding G, were removed in 2.3.0 after one release
# deprecated. They are recorded in releases/2.2.0.md and in the finding G
# ruling; nothing imports them.
#
# What one caller still needed from them was not stripping but LABELLING: the
# Triage panel shows every word a search ignored, and it must say whether a
# word was dropped because it is a company's legal form (never part of a mark,
# so there is nothing for staff to decide) or because it is weak (usually
# noise, occasionally the whole mark — DIRECT LINE — so staff can put it
# back). That question has its own function now.


@lru_cache(maxsize=1)
def _all_structural() -> frozenset:
    """Every legal form and corporate descriptor from every list, folded.

    Deliberately the UNION across jurisdictions, unlike stripping. Stripping
    has to be per-jurisdiction or a Norwegian AS eats an English preposition
    (finding G). Labelling does not: the question is "is this word a company
    form somewhere", the answer does not change what is searched, and the
    union is the safe direction — it can only move a word from "weak" to
    "structural", i.e. from offered-back to not-offered-back. A word wrongly
    offered back is a staff member unignoring LTD and wondering why their
    search filled with noise; a word wrongly withheld is a missing link.
    """
    forms = set(LEGAL_FORMS_GENERIC) | set(DESCRIPTIVE_SUFFIXES)
    for national in LEGAL_FORMS_BY_JURISDICTION.values():
        forms |= set(national)
    return frozenset(f.upper() for f in forms)


def is_structural(token: str) -> bool:
    """Is this word a company's legal form or corporate descriptor?

    True means "never part of the mark itself" — the word was dropped as a
    matter of fact, not judgement, and the Triage panel should not offer it
    back. See `legal_form_pattern` for the stripping side, which IS
    jurisdiction-specific and must stay that way.
    """
    t = str(token or "").strip().upper().rstrip(".")
    return bool(t) and t in _all_structural()
