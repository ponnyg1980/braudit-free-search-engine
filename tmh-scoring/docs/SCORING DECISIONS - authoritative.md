...
## H — Where sensitivity is applied, and by whom

**Ruling (Jonathan, 20 September 2026).** Sensitivity rules are applied at
**Triage / Client Review** level. They are saved within that report, the same
way exclusions are; they can be amended; and they can be cleared. Clearing is
dangerous, so it must be warned and logged.

**What follows from that, decided 21 September 2026 and shipped in 2.3.0:**

*Scoring settings are data, not code.* A threshold that only exists as a module
constant cannot be varied for one report without being varied for every report.
So every threshold moves onto a `ScoringSettings` object which travels with the
call. Omitting it gives the shipped calibration, which is why 2.3.0 is ZERO
DIFFERENCES on 56,690 records.

*Nothing written in Triage may change a global default.* This is enforced
structurally, not by policy: `DEFAULTS` lives in the package, and the only way
to move it is a package release with its own band_diff. Triage can produce a
delta from DEFAULTS for one run. It has no path to DEFAULTS itself.

*Thresholds are knobs; measurements are not.* Edit distance, syllable
splitting, Soundex, the overlap coefficient and the conflict matrix are the
model. They are not exposed and must not be.

*Sensitivity is per compartment.* Six: classes, terms/descriptions, sounds
like, spelled like, looks/reads alike, image. A change to one must not move
another — which required `mark_similarity`'s tier-3 gate to be tested per axis
rather than on the maximum of the three axes. Identical at the defaults, and
the reason the compartments mean anything at any other setting.

*Triage sees labelled steps, never numbers.* Nobody in Triage types 0.85. They
move a named step and 2.4.0's `sensitivity.py` maps it to a delta. R&D owns
what a step means, so changing that meaning later does not rewrite history: the
run recorded the step it used and the `scoring_version` that defined it.

**Still to build (app side, not the package):** `audit.run_sensitivity`
versioned per run — never updated in place, so "fuzzy +1, then +2 by whom and
when" survives; re-banding from the stored `components` rather than re-running
the search; a preview stating the consequence in rows before it is committed;
and a clear path that requires a reason, states what will move, leaves
exclusions untouched, and appends rather than deletes.

**Why the log matters more than the knob.** After a few weeks the join between
